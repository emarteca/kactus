...
expect(stash.kactusEntries).toHaveLength(0)
    })
...
const entry = await getLastKactusStashEntryForBranch(repository, 'master')
...
const lastEntry = stash.kactusEntries[0]

      
const actual = await getLastKactusStashEntryForBranch(
        repository,
        branchName
      )
...
expect(message).toBe('!!Kactus<master>')
    })
...
let entries = stash.kactusEntries
...
await dropKactusStashEntry(repository, stashToDelete.stashSha)
...
await dropKactusStashEntry(repository, doesNotExist.stashSha)
      } 
...
const { kactusEntries } = stash
        
expect(kactusEntries.length).toBe(1)
...
const entryToApply = kactusEntries[0]
...
expect(stashAfter.kactusEntries).not.toContain(entryToApply)
      })
...
var TIMING_TEMP_VAR_AUTOGEN495__RANDOM = perf_hooks.performance.now();
 await  FSE.writeFile(readme, generateString())
console.log("/home/ellen/Documents/ASJProj/TESTING_reordering/kactus/app/test/unit/git/stash-test.ts& [292, 8; 292, 53]& TEMP_VAR_AUTOGEN495__RANDOM& " + (perf_hooks.performance.now() - TIMING_TEMP_VAR_AUTOGEN495__RANDOM));
...
const result = await GitProcess.exec(
    ['stash', 'push', '-m', message || createKactusStashMessage(branchName)],
    repository.path
  )
