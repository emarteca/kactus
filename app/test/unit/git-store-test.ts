...
var TEMP_VAR_AUTOGEN120__RANDOM_LATER =  FSE.writeFile(readmeFilePath, 'SOME WORDS GO HERE\n')
...
await TEMP_VAR_AUTOGEN120__RANDOM_LATER
...
var TEMP_VAR_AUTOGEN157__RANDOM_LATER =  FSE.writeFile(readmeFilePath, 'WRITING SOME NEW WORDS\n')

    // setup requires knowing about the current tip
    
await gitStore.loadStatus([])
await TEMP_VAR_AUTOGEN157__RANDOM_LATER
...
var TEMP_VAR_AUTOGEN183__RANDOM =  gitStore.discardChanges([files[1]])
...
expect(files[0].status.kind).toEqual(AppFileStatusKind.Modified)
 ...
 await TEMP_VAR_AUTOGEN183__RANDOM
...
var TEMP_VAR_AUTOGEN412__RANDOM =  gitStore.discardChanges([files[0]])

expect(files.length).toEqual(1)

      

 await TEMP_VAR_AUTOGEN412__RANDOM
