...
const MaxStatusBufferSize = 20e6
// 20MB in decimal
...
if (entry.kind === 'copied' && oldPath != null) 
{
    
return { kind: AppFileStatusKind.Copied, oldPath }
  } 
else
if (entry.kind === 'renamed' && oldPath != null) 
{
...
var TIMING_TEMP_VAR_AUTOGEN400__RANDOM = perf_hooks.performance.now();
 var AWAIT_VAR_TIMING_TEMP_VAR_AUTOGEN400__RANDOM = await  isMergeHeadSet(repository)
console.log("/home/ellen/Documents/ASJProj/TESTING_reordering/kactus/app/src/lib/git/status.ts& [203, 2; 203, 57]& TEMP_VAR_AUTOGEN400__RANDOM& " + (perf_hooks.performance.now() - TIMING_TEMP_VAR_AUTOGEN400__RANDOM));
 const mergeHeadFound =  AWAIT_VAR_TIMING_TEMP_VAR_AUTOGEN400__RANDOM
  
const conflictedFilesInIndex = entries.some(
    e => conflictStatusCodes.indexOf(e.statusCode) > -1
  )
  
var TIMING_TEMP_VAR_AUTOGEN_CALLING_733_getRebaseInternalState__RANDOM = perf_hooks.performance.now();
 
var TIMING_TEMP_VAR_AUTOGEN_CALLING_721_getRebaseInternalState__RANDOM = perf_hooks.performance.now();
 

var TIMING_TEMP_VAR_AUTOGEN408__RANDOM = perf_hooks.performance.now();
 var AWAIT_VAR_TIMING_TEMP_VAR_AUTOGEN408__RANDOM = await  getRebaseInternalState(repository)
console.log("/home/ellen/Documents/ASJProj/TESTING_reordering/kactus/app/src/lib/git/status.ts& [207, 2; 207, 70]& TEMP_VAR_AUTOGEN408__RANDOM& " + (perf_hooks.performance.now() - TIMING_TEMP_VAR_AUTOGEN408__RANDOM));
 const rebaseInternalState =  AWAIT_VAR_TIMING_TEMP_VAR_AUTOGEN408__RANDOM
console.log("/home/ellen/Documents/ASJProj/TESTING_reordering/kactus/app/src/lib/git/status.ts& [207, 2; 207, 70]& TEMP_VAR_AUTOGEN_CALLING_721_getRebaseInternalState__RANDOM& " + (perf_hooks.performance.now() - TIMING_TEMP_VAR_AUTOGEN_CALLING_721_getRebaseInternalState__RANDOM));
 
console.log("/home/ellen/Documents/ASJProj/TESTING_reordering/kactus/app/src/lib/git/status.ts& [207, 2; 207, 70]& TEMP_VAR_AUTOGEN_CALLING_733_getRebaseInternalState__RANDOM& " + (perf_hooks.performance.now() - TIMING_TEMP_VAR_AUTOGEN_CALLING_733_getRebaseInternalState__RANDOM));
...
if ((match = value.match(/^branch.head (.*)/))) 
{
...
if ((match = value.match(/^branch.upstream (.*)/))) 
{
    
currentUpstreamBranch = match[1]
  } 
else
if ((match = value.match(/^branch.ab \+(\d+) -(\d+)$/))) 
{
